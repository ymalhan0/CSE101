...
// void equals(List A, List B); // Returns true iff Lists A and B are in same
// state, and returns false otherwise.

// Manipulation procedures
void clear(List L);        // Resets L to its original empty state.
void set(List L, void *x); // Overwrites the cursor element’s data with x. Pre:
                           // length()>0, index()>=0
...
void movePrev(List L); // If cursor is defined and not at front, move cursor one
                       ...
                       // and at // front, cursor becomes undefined; if cursor
...
void prepend(List L,
             void *x); // Insert new element into L. If L is non-empty, //
                       // insertion takes place before front element.
void append(List L,
            void *x); // Insert new element into L. If L is non-empty, //
                      // insertion takes place after back element.
void insertBefore(List L,
                  void *x); // Insert new element before cursor. // Pre:
                            // length()>0, index()>=0
void insertAfter(
    List L,
    void *x); // Insert new element after cursor. // Pre: length()>0, index()>=0
...
void printList(FILE *out,
               List L); // Prints to the file pointed to by out, a string rep
// of L consisting of a space separated seq of ints,
// with front on left.
...
// Optional operation
List concatList(List A,
                List B); // Returns a new List which is the concatenation of
