...
void Dictionary::LeftRotate(Node *N) {
  ...
  N->right = y->left;
  // turn ys left subtree into x's right subtree
  if (y->left != nil) {
    y->left->parent = N;
  }
  y->parent = N->parent; // link ys parent to x
  if (N->parent == nil) {
    root = y;
...
void Dictionary::RightRotate(Node *N) {
  Node *y = N->left;
  // y right subtree into x left subtree
  N->left = y->right;
  if (y->right != nil) {
    y->right->parent = N;
  }
...
// contains()
// Returns true if there exists a pair such that key==k, and returns
// false otherwise.
...
// ------------------------------------------------- clear() Resets
// this Dictionary to the empty state, containing no pairs.
...
// setValue()
// If a pair with key==k exists, overwrites the corresponding value
// with v, otherwise inserts the new pair (k, v).
...
// begin()
// If non-empty, places current iterator at the first (key, value)
// pair (as defined by the order operator < on keys), otherwise does
// nothing.
...
// (as defined by the order operator < on keys), otherwise does
// nothing.
void Dictionary::end() {
  if (num_pairs > 0) {
...
// ---------------------------------------------------------
// to_string()
// Returns a string representation of this Dictionary. Consecutive
// (key, value) pairs are separated by a newline "\n" character, and
// the items key and value are separated by the sequence
// space-colon-space " : ". The pairs are arranged in order, as
// defined by the order operator <.
...
// Consecutive keys are separated by newline "\n" characters. The key
// order is given by a pre-order tree walk.
...
// ---------------------------------------------------- operator<<()
// Inserts string representation of Dictionary D into stream, as
// defined by member function to_string().
...
// operator=()
// Overwrites the state of this Dictionary with state of D, and
